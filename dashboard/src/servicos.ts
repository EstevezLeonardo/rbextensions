...
/** Um item da lista devolvida por dashboard/servicos-inbox-listar.php. */
...
/** Formato da resposta de dashboard/servicos-inbox-listar.php. */
interface ListaInboxResposta {
    mensagens: MensagemInbox[];
    proximoPageToken: string | null;
}

/** Um anexo de arquivo (não imagem inline do template) listado em MensagemCompleta. */
interface Anexo {
    nome: string;
    tipo: string;
    tamanho: number;
...
/** Corpo completo devolvido por dashboard/servicos-inbox-ler.php. */
...
function reiniciarEPaginar(): void {
...
function ligarBotaoDeSair(): void {
    const botaoSair = document.getElementById('botao-email-sair');
    ...
    botaoSair.addEventListener('click', function () {
        const csrfToken = (document.getElementById('email-acoes-csrf-token') as HTMLInputElement).value;
...
function ligarAnexosDoEnvio(): void {
    const botaoAnexar = document.getElementById('botao-email-anexar');
    const inputAnexos = document.getElementById('email-anexos-input') as HTMLInputElement | null;
...
function desenharAnexosSelecionados(inputAnexos: HTMLInputElement, listaAnexos: HTMLElement): void {
...
function removerAnexoSelecionado(inputAnexos: HTMLInputElement, listaAnexos: HTMLElement, indiceRemovido: number): void {
...
function ligarFormularioDeEnvio(): void {
    const form = document.getElementById('form-email-envio') as HTMLFormElement | null;
    const mensagemEl = document.getElementById('email-envio-mensagem');
    const botaoEnviarEl = document.getElementById('botao-email-enviar') as HTMLButtonElement | null;
    const inputAnexos = document.getElementById('email-anexos-input') as HTMLInputElement | null;
    const listaAnexos = document.getElementById('email-anexos-lista');
    ...
    form.addEventListener('submit', function (evento) {
        ...
        const csrfToken = (document.getElementById('email-envio-csrf-token') as HTMLInputElement).value;
        const dadosDoFormulario = new FormData();
        dadosDoFormulario.append('destinatario', (document.getElementById('email-destinatario') as HTMLInputElement).value);
        dadosDoFormulario.append('assunto', (document.getElementById('email-assunto') as HTMLInputElement).value);
        dadosDoFormulario.append('mensagem', (document.getElementById('email-mensagem') as HTMLTextAreaElement).value);
        dadosDoFormulario.append('csrf_token', csrfToken);
        Array.from(inputAnexos?.files || []).forEach(function (arquivo) {
...
function ligarMenuDePastas(): void {
    const botoes = document.querySelectorAll<HTMLButtonElement>('.inbox-pasta-botao');
 ...
 */
function ligarFiltroDeData(): void {
    ...
    const dataInicioEl = document.getElementById('inbox-data-inicio') as HTMLInputElement | null;
    const dataFimEl = document.getElementById('inbox-data-fim') as HTMLInputElement | null;
    const buscaEl = document.getElementById('inbox-busca-texto') as HTMLInputElement | null;
...
function atualizarBotoesDePaginacao(temProximaPagina: boolean): void {
    const botaoAnterior = document.getElementById('botao-inbox-anterior') as HTMLButtonElement | null;
    const botaoProxima = document.getElementById('botao-inbox-proxima') as HTMLButtonElement | null;
 ...
 */
function carregarCaixaDeEntrada(): void {
    const listaEl = document.getElementById('lista-inbox');
    ...
    fetch('servicos-inbox-listar.php?' + parametros.toString())
        .then(function (resposta) {
            return resposta.json().then(function (dados) {
                ...
                return dados as ListaInboxResposta;
            });
        })
        .then(function (dados) {
 ...
 */
function alternarCorpoDaMensagem(uid: string, corpoEl: HTMLElement, botaoLer: HTMLButtonElement): void {
    ...
    fetch('servicos-inbox-ler.php?uid=' + encodeURIComponent(uid))
        .then(function (resposta) {
            return resposta.json().then(function (dados) {
                ...
                return dados as MensagemCompleta;
            });
        })
        .then(function (dados) {
...
function alternarOutrosItensDaLista(itemAberto: HTMLElement | null, esconderOutros: boolean): void {
 ...
 */
function desenharCorpoDaMensagem(dados: MensagemCompleta, corpoEl: HTMLElement): void {
    corpoEl.innerHTML = '';
...
function formatarTamanhoDeArquivo(bytes: number): string {
    if (bytes < 1024) {
        return bytes + ' B';
    }
    if (bytes < 1024 * 1024) {
